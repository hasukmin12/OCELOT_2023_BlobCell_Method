...
def _convert_format(pred_json, gt_json, num_images):
    ...
    for pred_cell in pred_json:
        ...
        step += 1
        if step == 1:
            img_idx_first = int(pred_cell["name"].split("_")[-1])
        img_idx = int(pred_cell["name"].split("_")[-1])
        if img_idx == 558:
            continue
        if img_idx_first != img_idx:
            print(img_idx, step, img_n)
            img_idx_first = img_idx  # 538 ~ 667 # 129개
            img_n += 1

        pred_after_convert[img_n].append((x, y, c, prob))
    ...
    for gt_cell in gt_json:
        ...
        step += 1
        if step == 1:
            img_idx_first = int(gt_cell["name"].split("_")[-1])
        img_idx = int(gt_cell["name"].split("_")[-1])
        if img_idx_first != img_idx:
            img_idx_first = img_idx
            img_n += 1

        gt_after_convert[img_n].append((x, y, c, prob))
...
def main():
    ...
    algorithm_output_path = "/home/sukmin/OCELOT_2023_BlobCell_Method_for_submission/ocelot23algo_for_submit_for_Github_submit_for_paper/test/output/val_GT/3_concat.json"
    with open(algorithm_output_path, "r") as f:
        pred_json = json.load(f)["points"]
    
    # Path where GT is stored
    gt_path = "/home/sukmin/OCELOT_2023_BlobCell_Method_for_submission/ocelot23algo_for_submit_for_Github_submit_for_paper/cell_gt_val.json"
    with open(gt_path, "r") as f:
        gt_json = json.load(f)["points"]
        # num_images = json.load(f)["num_images"]  # val : 137, test : 126
        num_images = 137 # val : 137, test : 126
