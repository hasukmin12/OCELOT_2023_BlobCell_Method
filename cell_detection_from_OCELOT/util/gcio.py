...
class DataLoader:
    ...
    def __init__(self, cell_path, tissue_path):
        ...
        # self.cell_patches = tif.imread(cell_fpath[0])
        # self.tissue_patches = tif.imread(tissue_fpath[0])
    ...
    def __next__(self):
        if self.cur_idx < self.num_images:
            ...
            pair_id = int(self.cell_fpath[self.cur_idx].split("_")[-1].split(".")[0])
...
class DataLoader_with_GT:
    ...
    tissue_path: Path
        Path to where the tissue patches can be found
    """
    def __init__(self, cell_path, tissue_path, blobcell_path):
        # cell_fpath = [os.path.join(cell_path, f) for f in os.listdir(cell_path) if ".tif" in f]
        # tissue_fpath = [os.path.join(tissue_path, f) for f in os.listdir(tissue_path) if ".tif" in f]
        # assert len(cell_fpath) == len(tissue_fpath) == 1
        ...
        self.cell_fpath = [os.path.join(cell_path, f) for f in os.listdir(cell_path) if ".png" in f]
        self.tissue_fpath = [os.path.join(tissue_path, f) for f in os.listdir(tissue_path) if ".png" in f]
        self.blobcell_fpath = [os.path.join(blobcell_path, f) for f in os.listdir(blobcell_path) if ".png" in f]
        # assert len(cell_fpath) == len(tissue_fpath) == 1

        self.cell_patches = np.array(Image.open(self.cell_fpath[0]))
        self.tissue_patches = np.array(Image.open(self.tissue_fpath[0]))
        self.blobcell_patches = np.array(Image.open(self.blobcell_fpath[0]))
    ...
    def __next__(self):
        if self.cur_idx < self.num_images:
            ...
            cell_patch = np.array(Image.open(self.cell_fpath[self.cur_idx]))
            tissue_patch = np.array(Image.open(self.tissue_fpath[self.cur_idx]))
            blobcell_patch = np.array(Image.open(self.blobcell_fpath[self.cur_idx]))
            ...
            # Return the image data
            return cell_patch, tissue_patch, blobcell_patch, pair_id
        else:
            # Raise StopIteration when no more images are available
            raise StopIteration
