...
class DataLoader:
    ...
    def __init__(self, cell_path, tissue_path):
        ...
        self.cell_fpath = [os.path.join(cell_path, f) for f in os.listdir(cell_path) if ".jpg" in f]
        self.tissue_fpath = [os.path.join(tissue_path, f) for f in os.listdir(tissue_path) if ".jpg" in f]
        # assert len(cell_fpath) == len(tissue_fpath) == 1
