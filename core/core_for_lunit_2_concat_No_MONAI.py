...
def validation(info, config, valid_loader, model, logging=False, threshold=0.5):  
    ...
    with torch.no_grad():
        for step, batch_set in enumerate(epoch_iterator_val):
            ...
            x, val_labels, tissue, cell = batch_set
            ...
            x = x.to("cuda:{0}".format(int(info["GPUS"])))/255
            val_labels = val_labels.to("cuda:{0}".format(int(info["GPUS"])))
            tl = tissue.to("cuda:{0}".format(int(info["GPUS"])))
            # cl = cell.to("cuda:{0}".format(int(info["GPUS"])))

            val_inputs = torch.cat((x,tl), dim=1)
            # intense_val_inputs = val_inputs/(val_inputs.max())  # ScaleIntensity 대신 추가
            intense_val_inputs = val_inputs  # ScaleIntensity 대신 추가
...
def train(info, config, global_step, dice_val_best, steps_val_best, dice_val_best_class, steps_val_best_class, model, optimizer, train_loader, valid_loader, logging=False, deep_supervision=True): 
    ...
    # aim_path = "/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/Test_in_training/image4"
    # aim_path2 = "/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/Test_in_training/tissue4"
    # aim_path3 = "/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/Test_in_training/cell4"
    ...
    for step, batch_set in enumerate(epoch_iterator):
        ...
        x, y1, tissue, cell = batch_set

        x = x.to("cuda:{0}".format(int(info["GPUS"])))/255
        y1 = y1.to("cuda:{0}".format(int(info["GPUS"])))
        tl = tissue.to("cuda:{0}".format(int(info["GPUS"])))
        # cl = cell.to("cuda:{0}".format(int(info["GPUS"])))
        ...
        intense_input = input # (8, 5, 1024, 1024)
        ...
        # 0.5k마다 model save
        if global_step % 3000 == 0 and global_step > 3000 :
            torch.save({
        ...
        if (
            ...
            if dice_val > dice_val_best:
                ...
                torch.save({
                    ...
                    "model_state_dict": model.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                }, os.path.join(info["LOGDIR"], "model_best_e{0:05d}_dice{1}.pth".format(global_step, dice_val)))
                print(
                ...
                torch.save({
                    ...
                    "model_state_dict": model.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                }, os.path.join(info["LOGDIR"], "model_e{0:05d}_dice{1}.pth".format(global_step, dice_val)))
                ...
                print(
                    f"Model Was Saved ! But Current Best Avg. Dice: {dice_val_best} Current Avg. Dice: {dice_val} Best model step is {steps_val_best}" 
                )
            ...
            if dice_val_class > dice_val_best_class:
                ...
                torch.save({
                    ...
                    "model_state_dict": model.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                }, os.path.join(info["LOGDIR"], "class_model_best_e{0:05d}_dice{1}.pth".format(global_step, dice_val_class)))
                print(
