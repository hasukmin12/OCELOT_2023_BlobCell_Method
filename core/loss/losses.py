...
class CE_Loss(_Loss):
    ...
    def __init__(
        ...
        """
        super().__init__()
        reduction = look_up_option(reduction, LossReduction).value
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        total_loss: torch.Tensor = ce_loss

        return total_loss



class DiceLoss_Portion(_Loss):
    """
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(f)):
            ...
            f[i][0] = f[i][0] * 0.1524771395423697
            f[i][1] = f[i][1] * 0.8475228604576303
        ...
        #     f[i][0] = f[i][0] * 0.3295816338524442
        #     f[i][1] = f[i][1] * 0.013716344076734093
        #     f[i][2] = f[i][2] * 0.6082988732715413
        #     f[i][3] = f[i][3] * 0.04840314879928037


        # # for Multi_Organ
        # for i in range(len(f)):

        #     f[i][0] = f[i][0] * 0.014857976623663963
        #     f[i][1] = f[i][1] * 0.03142741599883561
        #     f[i][2] = f[i][2] * 0.23585654794731511
        #     f[i][3] = f[i][3] * 0.5995713295643015
        #     f[i][4] = f[i][4] * 0.11828672986588397
...
class DiceCELoss_Portion(_Loss):
    """
    Compute both Dice loss and Cross Entropy Loss, and return the weighted sum of these two losses.
    The details of Dice loss is shown in ``monai.losses.DiceLoss``.
    The details of Cross Entropy Loss is shown in ``torch.nn.CrossEntropyLoss``. In this implementation,
...
class DiceFocalLoss_Portion(_Loss):
    """
