...
img_path = "/vast/AI_team/sukmin/datasets/Lunit_Challenge_for_paper/labels/train/tissue"
img_list = sorted(next(os.walk(img_path))[2])

aim_path = "/vast/AI_team/sukmin/datasets/Lunit_Challenge_for_paper/labels/train/tissue_crop"
...
for case in img_list:
    if case[-4:]==".png":
        ...
        # cy = meta['sample_pairs']["{0:03d}".format(int(img_list.index(case)+81))]['patch_y_offset']
                        cx = meta['sample_pairs']["{0:03d}".format(int(img_list.index(case)+1))]['patch_x_offset'] # +1 , +401, +538 
